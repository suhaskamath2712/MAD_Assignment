...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getSharedPreferences("FoodDeliverySharedPrefs", MODE_PRIVATE).contains("login"))    login = getSharedPreferences("FoodDeliverySharedPrefs",MODE_PRIVATE).getString("login", "");
        else                                                                                         login = null;
        randomView();
    }

    //NAME: randomView
    //PURPOSE: Display the random list of food.
    //INPUTS: NONE
    //OUTPUTS: NONE
    private void randomView ()
    {
        recyclerView.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
        recyclerView.setAdapter(new FoodAdapter(random,MainActivity.this, 1));
        ...
        if (login == null)
            total.setText("You are not logged in.");
        else
            total.setText("You are logged in as " + login + ".");
    }

    //NAME: restaurantView
    //PURPOSE: Display the list of restaurants.
    //INPUTS: NONE
    //OUTPUTS: NONE
    private void restaurantView ()
    {
        ...
        button1.setVisibility(View.VISIBLE);
        button1.setOnClickListener(v -> randomView());
        button2.setText("BUCKET");
        button2.setVisibility(View.VISIBLE);
        button2.setOnClickListener(v -> bucketView());
    ...
    //NAME: bucketView
    //PURPOSE: Display the list of food items in the bucket.
    //INPUTS: NONE
    //OUTPUTS: NONE
    private void bucketView ()
    {
        ...
        button2.setVisibility(View.VISIBLE);
        button1.setOnClickListener(v -> restaurantView());
        total.setText("Your total is: $" + (double) Math.round(calculateTotal()*100)/100 + ".");

        if (login == null)  button2.setOnClickListener(v -> login());
        else                button2.setOnClickListener(v -> checkoutSuccess());
    }

    //NAME: foodView
    //PURPOSE: Display the list of food for a particular restaurant.
    //INPUTS: Restaurant, of which the food items need to be displayed
    //OUTPUTS: NONE
    ...
    //NAME: login
    //PURPOSE: Background code for the user to complete logging in.
    //INPUTS: NONE
    //OUTPUTS: NONE
    private void login()
    {
        ...
        {
            if (email.getText().toString().length() == 0 || !Pattern.compile("^(.+)@(.+)$").matcher(email.getText().toString()).matches())
                Toast.makeText(getApplicationContext(), "Please enter your email properly!", Toast.LENGTH_LONG).show();
            else if (!helper.emailExists(email.getText().toString()))
            {
                if (pass.getText().toString().length() != 0)
                {
                    ...
                    getSharedPreferences("FoodDeliverySharedPrefs",MODE_PRIVATE).edit().putString("login", login).commit();
                    checkoutSuccess();
                }
                else
            ...
            {
                if (helper.correctPass(email.getText().toString(), pass.getText().toString()))
    ...
    //PURPOSE: The message to display when the checkout is successful.
    //INPUTS: NONE
    //OUTPUTS: NONE
    ...
    //NAME: past
    //PURPOSE: Used to display the previous orders made by the user. Works only if the user is logged in.
    //INPUTS: NONE
    //OUTPUTS: NONE
    private void past ()
    {
    ...
    //NAME: calculateTotal
    //PURPOSE: Used to calculate the total value of the items in the bucket.
    //INPUTS: NONE
    //OUTPUTS: total - The total price of items in the bucket.
    ...
    private class RestaurantAdapter extends RecyclerView.Adapter<RestaurantAdapter.MyViewHolder>
    {
        ...
        public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            ...
            holder.picture.setImageResource(getResources().getIdentifier(restaurantList.get(position).getImage(), "drawable", getPackageName()));
    ...
    private class FoodAdapter extends RecyclerView.Adapter<FoodAdapter.MyViewHolder>
    {
        ...
        public void onBindViewHolder(@NonNull FoodAdapter.MyViewHolder holder, int position) {
            ...
            holder.picture.setImageResource(getResources().getIdentifier(foodList.get(position).getImage(), "drawable", getPackageName()));
